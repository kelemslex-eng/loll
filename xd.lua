...
print("NOT: Script yüklendi! UI için sağ üstteki Kavo menüyü aç, toggle'larla özellikleri aktif et.")
...
local function toggleProtectedNoclip(state)
    ...
    if state then
        ...
        print("NOT: Protected Noclip aktif! Duvarlardan geçmek için toggle açık tut.")
    else
...
local function toggleInvisibleUnderground(state)
    ...
    if state then
                            ...
                            for _, brainrot in pairs(base:GetChildren()) do
                                if string.lower(brainrot.Name):find("brainrot") or brainrot.Name:match("Secret") or brainrot.Name:match("God") then
                                    ...
                                    if stealEvent then
                                        ...
                                        print("NOT: Underground'dan " .. brainrot.Name .. " çalındı! Görünmez modda.")
                        ...
                        for _, obj in pairs(conveyor:GetChildren()) do
                            if obj.Name:match("Brainrot") then
                                ...
                                if stealEvent then
                                    ...
                                    print("NOT: Conveyor'dan " .. obj.Name .. " çalındı! Underground modda.")
        ...
        print("NOT: Invisible Underground aktif! Toggle açık tut, yerin altına gider ve çalarsın.")
    else
        for _, part in pairs(character:GetDescendants()) do
            if part:IsA("BasePart") then
                part.Transparency = 0
...
local function toggleInfRizz(state)
    ...
    if state then
        ...
        print("NOT: Infinite Rizz/Cash aktif! Sınırsız para kazanırsın.")
    end
end
...
local function toggleAutoRebirthMulti(state)
    ...
    if state then
        ...
        print("NOT: Auto Rebirth Multiplier aktif! x10 boost ile rebirth yap.")
    end
end
...
local function toggleServerHop(state)
    ...
    if state then
        ...
        print("NOT: Server Hop aktif! Full serverlara atlar.")
    end
end
...
local function toggleMutationDetect(state)
    ...
    if state then
        ...
        print("NOT: Mutation Detect aktif! Mutasyonlu brainrotları bildirir.")
    end
end
...
local function toggleSecretSpawner(state)
    ...
    if state then
        ...
        print("NOT: Secret Spawner aktif! Secret brainrot spawnlar.")
    end
end
...
local function toggleWebSlinger(state)
    ...
    if state then
        ...
        print("NOT: Web Slinger aktif! F tuşu ile fling yap.")
    else
...
local function toggleInfJump(state)
    ...
    if state then
        ...
        print("NOT: Infinite Jump aktif! Zıplamak için sürekli bas.")
    else
        if connections.infJump then connections.infJump:Disconnect() end
        print("NOT: Infinite Jump kapatıldı.")
    end
end
...
local function toggleGlide(state)
    ...
    if state then
        ...
        print("NOT: Glide aktif! Yavaş düşmek için toggle açık tut.")
    else
...
local function toggleKeypadAuto(state)
    ...
    if state then
        ...
        print("NOT: Keypad Auto aktif! Base keypad'leri otomatik kır.")
    end
end
...
local function toggleAutoEvent(state)
    ...
    if state then
        ...
        print("NOT: Auto Event aktif! Shamrock/Meteor eventlerine katılır.")
    end
end
...
local function toggleAutoSteal(state)
    ...
    if state then
        ...
        print("NOT: Auto Steal aktif! Conveyor ve base'den brainrot çalar.")
    end
end
...
local function toggleAutoLock(state)
    ...
    if state then
        ...
        print("NOT: Auto Lock aktif! Base her 30 sn kilitlenir.")
    end
end
...
local function toggleAutoBuy(state)
    ...
    if state then
        ...
        print("NOT: Auto Buy aktif! Secret itemler alınır.")
    end
end
...
local function toggleAutoRebirth(state)
    ...
    if state then
        ...
        print("NOT: Auto Rebirth aktif! Otomatik rebirth yapar.")
    end
end
...
local function toggleFly(state)
    ...
    if state then
        ...
        print("NOT: Fly aktif! WASD ile uç, Space yukarı, Shift aşağı.")
    else
...
local function toggleSpeed(state)
    ...
    if state then
        ...
        print("NOT: Speed Boost aktif! Hız " .. speedValue .. " olarak ayarlandı.")
    else
        humanoid.WalkSpeed = 16
        print("NOT: Speed Boost kapatıldı, normal hız 16.")
    end
end
...
local function toggleESP(state)
    ...
    if state then
        ...
        print("NOT: ESP aktif! Brainrot'lar yeşil glow ile görünür.")
    else
        for _, obj in pairs(workspace:GetChildren()) do
...
local function togglePlayerESP(state)
    ...
    if state then
        ...
        print("NOT: Player ESP aktif! Diğer oyuncular kırmızı glow ile görünür.")
    else
        for _, plr in pairs(Players:GetPlayers()) do
...
local function toggleAutoUpgrade(state)
    ...
    if state then
        ...
        print("NOT: Auto Upgrade aktif! Base otomatik yükseltir.")
    end
end
...
local function toggleFastRespawn(state)
    ...
    if state then
        ...
        print("NOT: Fast Respawn aktif! Ölünce hemen doğar.")
    end
end

local MainTab = Window:NewTab("🚀 Ana Özellikler")
local MainSection = MainTab:NewSection("Auto Farm & Steal")
MainSection:NewToggle("Auto Steal Brainrots", "Otomatik brainrot çal", toggleAutoSteal)
...
MainSection:NewToggle("Auto Rebirth", "Otomatik rebirth", toggleAutoRebirth)
MainSection:NewToggle("Invisible + Underground Steal", "Görünmez yer altı çalma", toggleInvisibleUnderground)
...
AdvancedSection:NewToggle("Web Slinger/Fling", "Fling (F tuşu)", toggleWebSlinger)
...
player.CharacterAdded:Connect(function(newChar)
    ...
    print("NOT: Karakter yenilendi, aktif özellikler geri yüklendi.")
end)

print("NOT: Script hazır! Hata alırsan F9 ile konsolu aç, hatayı buraya yaz, hemen çözerim kanka!")